...
function renderSettings() {
  ...
  setVal('set-apikey',p.apiKey||'');
  // JSONBin
  try{
    const cfg=JSON.parse(localStorage.getItem('fitcoach_jsonbin')||'{}');
    setVal('set-jb-key',cfg.key||'');
    setVal('set-jb-binid',cfg.binId||'');
  }catch{}
  // Custom sports
  renderCustomSports();
}
...
function saveSettings() {
  ...
  // JSONBin config (saved separately, not in state)
  const jbKey=(document.getElementById('set-jb-key')?.value||'').trim();
  const jbBinId=(document.getElementById('set-jb-binid')?.value||'').trim();
  if(jbKey&&jbBinId){
    localStorage.setItem('fitcoach_jsonbin',JSON.stringify({key:jbKey,binId:jbBinId}));
    showToast('☁️ JSONBin configuré !','info');
  }
  saveState();
...
async function testMistralKey() {
  ...
  const showResult = (ok, msg) => {
    if (resultEl) {
      ...
      resultEl.style.color = ok ? 'var(--accent-green)' : 'var(--accent-fire)';
    }
    showToast(msg, ok ? 'success' : 'error');
    if (btn) { btn.textContent = '🔍 Tester'; btn.disabled = false; }
  };

  // Use AbortController for timeout (10s)
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 10000);

  try {
    // Use chat/completions — same endpoint the app uses, CORS-safe
    ...
    if (r.ok) {
      ...
      const reply = data.choices?.[0]?.message?.content || '(réponse reçue)';
      showResult(true, `✅ Clé valide ! Réponse : "${reply.trim()}"`);
    } else {
      let errMsg = r.statusText;
      try { const j = await r.json(); errMsg = j.message || j.error?.message || errMsg; } catch {}
      if (r.status === 401) showResult(false, '❌ Clé invalide ou révoquée (401). Régénérez-la sur console.mistral.ai');
      else if (r.status === 422) showResult(false, `❌ Requête invalide (422): ${errMsg}`);
      else if (r.status === 429) showResult(false, '⏳ Quota dépassé (429). Attendez ou vérifiez votre plan.');
      else showResult(false, `❌ Erreur ${r.status}: ${errMsg}`);
    }
  } catch (e) {
    clearTimeout(timeout);
    if (e.name === 'AbortError') {
      showResult(false, '⏱️ Timeout (10s). Vérifiez votre connexion internet.');
    } else if (e.message?.includes('Failed to fetch') || e.message?.includes('NetworkError')) {
      showResult(false, '🌐 Impossible de joindre api.mistral.ai. Si vous êtes en file://, ouvrez l\'app via un serveur local (ex: npx serve .)');
    } else {
      showResult(false, `❌ Erreur: ${e.message}`);
    }
  }
}
...
async function setupJSONBin() {
  const key = (document.getElementById('set-jb-key')?.value||'').trim();
  if(!key){ showToast('⚠️ Entrez votre clé JSONBin d\'abord','error'); return; }
  const btn = document.getElementById('btn-jb-create');
  if(btn){ btn.innerHTML='<span class="spinner"></span> Création...'; btn.disabled=true; }
  try {
    const binId = await createJSONBin(key);
    document.getElementById('set-jb-binid').value = binId;
    localStorage.setItem('fitcoach_jsonbin', JSON.stringify({key, binId}));
    const el = document.getElementById('jb-setup-result');
    if(el){ el.textContent = `✅ Bin créé ! ID: ${binId} — Sauvegardez vos paramètres.`; el.style.color='var(--accent-green)'; el.style.display='block'; }
    showToast('☁️ JSONBin créé et configuré !','success');
  } catch(e) {
    const el = document.getElementById('jb-setup-result');
    if(el){ el.textContent = '❌ Erreur: '+e.message; el.style.color='var(--accent-fire)'; el.style.display='block'; }
    showToast('❌ Erreur JSONBin: '+e.message,'error');
  }
  if(btn){ btn.textContent='🆕 Créer un Bin'; btn.disabled=false; }
}

async function testJSONBinSync() {
  const synced = await loadFromCloud();
  if(synced) showToast('☁️ Synchronisation réussie !','success');
  else showToast('❌ Sync échouée — vérifiez clé et Bin ID','error');
}
